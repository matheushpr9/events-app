...
const Index = () => {
    ...
    useEffect(() => {
        if (subscriptionStatus === null) return; // Aguarda o status ser carregado
        ...
        getUserInfo().then((authenticatedUser: AuthenticatedUser) => {
            userHasSpaces(authenticatedUser.user.id).then((hasSpaces) => {
                if (subscriptionStatus && !subscriptionStatus.isActive && hasSpaces) {
                    toast(
                        <div>
                            Você possui espaços disponíveis, mas sua assinatura está inativa. Por favor, ative sua assinatura para gerenciar seus espaços.
                            <br />
                            <button
